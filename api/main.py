                ...
                "runways": [
                    {
                        "id": rw.id,
                        "heading": rw.heading,
                        "start": config_handler.xy_to_latLon_list(
                            rw.start.x, rw.start.y, ap.anchor
                        ),
                        "end": config_handler.xy_to_latLon_list(
                            rw.end.x, rw.end.y, ap.anchor
                        ),
                    }
...
async def broadcast_external_state(payload: dict[str, Any]):
    message = dict(payload)
    message.setdefault("source", "rl_env_inference")
...
async def parse_and_process_command_str(payload: str):
    ...
    try:
        ...
        if cmd_prefix == "ATC" and len(parts) >= 3:
            ...
            if cmd_id == "ATC_ALTITUDE" and value:
                req_data["new_altitude"] = float(value)
            elif cmd_id == "ATC_SPEED" and value:
                req_data["new_speed"] = float(value)
            elif cmd_id == "ATC_DIRECT_TO" and value:
            ...
            if "error" in result:
                engine.event_buffer.append(
                    {"type": "ERROR", "msg": result["error"], "timestamp": time.time()}
                )
        ...
        engine.event_buffer.append(
            {"type": "ERROR", "msg": err_msg, "timestamp": time.time()}
        )
        return err_msg
        ...
        while True:
            ...
            try:
                if msg_type == "create_airport":
                    ...
                    ap_code, wp_id = (
                        payload.get("airport_code") or "VOCB",
                    ...
                    is_departure = (
                        str(payload.get("is_departure", "false")).lower() == "true"
                    )
                    gate, callsign, ac_type = (
                        payload.get("gate", "N"),
                        payload.get(
                            "callsign", f"SQ{random.randint(100, 999)}"
                        ).upper(),
                        payload.get("type", "B738"),
                    )
                    if is_departure:
                        engine.spawn_departure(
                            callsign,
                            ac_type,
                            payload.get("runway_id") or "RWY_1",
                            gate,
                            payload.get("terminal_gate_id"),
                        )
                    else:
                        engine.add_aircraft(
                            callsign,
                            ...
                            "Medium",
                            gate,
                            payload.get("altitude", 10000),
                            payload.get("speed", 250),
                        )
...
async def process_command(request: CommandRequest):
    if request.type == CommandType.SIMULATION:
        if (
            request.command_id == "CMD_SET_TIME_SCALE"
            and request.time_scale is not None
        ):
    ...
    if cmd == "ATC_ALTITUDE" and request.new_altitude is not None:
        ...
        engine.event_buffer.append(
            {
                "type": "ATC",
                "msg": f"ALTITUDE: {request.callsign} -> {alt}ft (manual)",
                "timestamp": time.time(),
            }
        )
        ...
        engine.event_buffer.append(
            {
                "type": "ATC",
                "msg": f"SPEED: {request.callsign} -> {spd}kts (manual)",
                "timestamp": time.time(),
            }
        )
        ...
        if engine.config:
            ...
            for key, name in engine.config.star_names.items():
                if name.upper() == search_term:
                    ...
                    if (
                        gate_id in engine.config.stars
                        and rwy_id in engine.config.stars[gate_id]
                    ):
                        (
                            aircraft.active_star,
                            aircraft.wp_index,
                            aircraft.target_runway_id,
                            aircraft.state,
                        ) = gate_id, 0, rwy_id, "ENROUTE"
                        engine.event_buffer.append(
                            {
                                "type": "ATC",
                                "msg": f"DIRECT: {request.callsign} proc {name} for RWY {rwy_id}",
                                "timestamp": time.time(),
                            }
                        )
            ...
            for key, name in engine.config.sid_names.items():
                if name.upper() == search_term:
                    ...
                    if sid_wp_ids:
                        ...
                        aircraft.active_route, aircraft.route_index, aircraft.state = (
                            route,
                            0,
                            "CLIMB_OUT",
                        )
                        engine.event_buffer.append(
                            {
                                "type": "ATC",
                                "msg": f"DIRECT: {request.callsign} sid {name} to {gate_id}",
                                "timestamp": time.time(),
                            }
                        )
                        return {"status": "success"}
            ...
            found_wp = next(
                (
                    ...
                    if (wp.name and wp.name.upper() == search_term)
            ...
            if found_wp:
                ...
                if aircraft.active_star and aircraft.active_star in engine.config.stars:
                    ...
                    rwy_id = (
                        aircraft.target_runway_id
                        or list(engine.config.stars[aircraft.active_star].keys())[0]
                    )
                    route_ids = engine.config.stars[aircraft.active_star].get(
                        rwy_id, []
                    )
                    try:
                ...
                engine.event_buffer.append(
                    {
                        "type": "ATC",
                        "msg": f"DIRECT: {request.callsign} to {found_wp.name}",
                        "timestamp": time.time(),
                    }
                )
                return {"status": "success"}
        ...
        engine.event_buffer.append(
            {
                "type": "ATC",
                "msg": f"RESUME: {request.callsign} (manual overrides cleared)",
                "timestamp": time.time(),
            }
        )
    elif cmd == "ATC_LAND":
        if request.runway_id and engine.config:
            ...
            new_route, msg_suffix = (
                engine.config.stars.get(entry_gate, {}).get(rw_id),
                "",
            )
            if new_route:
            ...
            engine.event_buffer.append(
                {
                    "type": "ATC",
                    "msg": f"CLEARED LAND: {request.callsign} RWY {rw_id}{msg_suffix}",
                    "timestamp": time.time(),
                }
            )
        else:
        ...
        if request.runway_id:
            ...
            if target_rw:
                (
                    ...
                    aircraft.runway_heading,
                ) = (
                    rw_id,
                    {"x": target_rw.start.x, "y": target_rw.start.y},
                    target_rw.heading,
                )
        ...
        engine.event_buffer.append(
            {
                "type": "ATC",
                "msg": f"TAXI: {request.callsign} to RWY {aircraft.target_runway_id}",
                "timestamp": time.time(),
            }
        )
        ...
        engine.runway_status[aircraft.target_runway_id]["occupied_by"] = (
            request.callsign
        )
        engine.event_buffer.append(
            {
                "type": "ATC",
                "msg": f"LINE-UP: {request.callsign} RWY {aircraft.target_runway_id}",
                "timestamp": time.time(),
            }
        )
        ...
        if aircraft.state == "TAXIING":
            ...
            engine.event_buffer.append(
                {
                    "type": "ATC",
                    "msg": f"QUEUED TAKEOFF: {request.callsign}",
                    "timestamp": time.time(),
                }
            )
            ...
            engine.event_buffer.append(
                {
                    "type": "ATC",
                    "msg": f"CLEARED TAKEOFF: {request.callsign} (Aligning)",
                    "timestamp": time.time(),
                }
            )
            ...
            engine.event_buffer.append(
                {
                    "type": "ATC",
                    "msg": f"CLEARED TAKEOFF: {request.callsign}",
                    "timestamp": time.time(),
                }
            )
        else:
